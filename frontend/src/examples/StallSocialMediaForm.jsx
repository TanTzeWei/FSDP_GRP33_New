...
import './StallSocialMediaForm.css';

/**
 * Example: Form for stall owners to add/edit their social media links
 * 
 * This form allows stall owners to:
 * 1. Add their social media URLs
 * 2. Update existing URLs
 * 3. See preview of social icons
 */

const StallSocialMediaForm = ({ stallId }) => {
  ...
  useEffect(() => {
    const fetchStallData = async () => {
      try {
        const response = await axios.get(`/api/stalls/${stallId}`);
        const stall = response.data.stall;
  ...
  const handleSubmit = async (e) => {
    ...
    for (const [key, value] of Object.entries(socialMedia)) {
      if (value && !validateUrl(value)) {
        setError(`Invalid URL for ${key.replace('_url', '')}`);
        setLoading(false);
        return;
      }
    }
    ...
    try {
      await axios.put(
        `/api/stalls/${stallId}/social-media`,
        socialMedia,
        {
          headers: {
  ...
  return (
    <div className="social-media-form-container">
      ...
      <p className="form-description">
        Add your social media profiles to help customers find and follow you online.
      </p>
...
export default StallSocialMediaForm;
